...
export function ScoutCard({ symbol, dataReady }: ScoutCardProps) {
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <span className="text-lg font-semibold text-gray-100">Scout</span>
          <span className="text-xs text-gray-500 font-medium tracking-wide">AI Research Assistant</span>
          {cached && (
            <span
      ...
      {/* Loading */}
      {loading && (
        <div className="flex items-center gap-2 text-gray-400 text-sm" data-testid="scout-loading">
          <span className="animate-pulse">Scout is researching {symbol}...</span>
        </div>
      )}

      {/* Error */}
      {error && (
        <div className="text-negative text-sm" data-testid="scout-error">
          {error}
        </div>
      )}
      ...
      {/* Streamed text */}
      {text && (
        <div className="text-sm text-gray-300 leading-relaxed" data-testid="scout-text">
      ...
      {text && !isStreaming && (
        <p className="text-xs text-gray-600 mt-3 border-t border-white/5 pt-2" data-testid="disclaimer">
          Scout AI summary — not financial advice.
        </p>
      )}
    </div>
  )
